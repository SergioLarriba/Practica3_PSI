...
class ChessGame(models.Model):
    ...
    timeControl = models.CharField(max_length=15, default='30')  #Tiempo por movimiento  ¿¿CHAR???????????
    whitePlayer = models.ForeignKey(Player, on_delete=models.CASCADE, related_name='whitePlayer',null=True)
    blackPlayer = models.ForeignKey(Player, on_delete=models.CASCADE, related_name='blackPlayer',null=True)
    ...
    def __str__(self):
        ...
        whitePlayer = f'{self.whitePlayer.username} ({self.whitePlayer.rating})' if self.whitePlayer else 'unknown'
...
class ChessMove(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        move = chess.Move.from_uci(self.move_from + self.move_to + (self.promotion if self.promotion not in [None, ''] else ''))
    ...
    def __str__(self):
        if self.promotion:
            return f'{self.player} {self.move_from} -> {self.move_to} {self.promotion}'
